...
public partial class DetalleReservaPage : ContentPage
{
    ...
    private async Task CargarDetalleAsync()
    {
        try
        {
            ...
            RutaLabel.Text = $"Ruta: {detalle.Ruta?.Nombre}";
            EstadoLabel.Text = $"Estado: {detalle.EstadoTexto}";
            OrigenLabel.Text = $"Origen: {detalle.Ruta?.Origen}";
            DestinoLabel.Text = $"Destino: {detalle.Ruta?.Destino}";
            UnidadLabel.Text = $"Unidad: {detalle.Unidad?.Placa} - {detalle.Unidad?.Modelo}";
            ChoferLabel.Text = $"Chofer: {detalle.Chofer?.NombreCompleto}";
            ...
            AsientoLabel.Text = $"Número de asiento: {_reserva.NumeroAsiento}";

            decimal precioBase = detalle.PrecioBase > 0
                ? detalle.PrecioBase
                : detalle.Ruta?.PrecioBase ?? 0;

            decimal montoPagado = detalle.MontoPagado > 0
                ? detalle.MontoPagado
                : _reserva.MontoPagado;

            PrecioBaseLabel.Text = $"Precio base: ₡{precioBase:N2}";
            MontoPagadoLabel.Text = $"Monto total pagado: ₡{montoPagado:N2}";
        }
        catch (Exception ex)
        {
